...
from PIL import Image as Scrimmage
import numpy as pp
import sys as ter
import sys as ter
...
# import line_profiler as cocaine
# cocaine = cocaine.LineProfiler()

# ter.argv = [ter.argv[0],0,0,0]
# ter.argv[1] = 'biblioblex'
# ter.argv[3] = 'Rumpletitskin wants to meet you'
# ter.argv[2] = 'https://cdn.discordapp.com/attachments/698318797257441336/900109830897942528/1200px-Dennys_logo.svg.png'
# ter.argv[2] = 'https://cdn.discordapp.com/attachments/698318797257441336/899501583149199420/ben.png'
# ter.argv[2] = 'https://cdn.discordapp.com/attachments/698318797257441336/900095476890742794/Mac_Shea.jpg'
...
class Squectangle():
    ...
    def whereamI(self):
        abcdef = pp.array([self.tall, self.ball, self.tar, self.bar])
        return abcdef.min(axis = 0).astype('int'), abcdef.max(axis = 0).astype('int') + 1
...
def biblio():
    ...
    # find the extents of the bowlgerale
    ter.stderr.write("biblio--5\n"); ter.stderr.flush()
    bowlgerale_mins, bowlgerale_maxs = desquircularized_bowlgerale.whereamI()
    bowlgerale_slice = slice(bowlgerale_mins[1],bowlgerale_maxs[1]), slice(bowlgerale_mins[0], bowlgerale_maxs[0])
    # blend the sauces into the ward
    ter.stderr.write("biblio--6\n"); ter.stderr.flush()
    sauceinthegerale = embrasser.jambossthejuice(
        pp.array(cream_of_corn)[bowlgerale_slice],
        sauceingredients[bowlgerale_slice],
        loomingshit[bowlgerale_slice])
    ter.stderr.write("biblio--7\n"); ter.stderr.flush()
    mesk = pp.array(corn_hole(
            Scrimmage.new('L', (scrimmage.width, scrimmage.height), (255)),
            *howto_cornhole))[bowlgerale_slice]
    mesk = Scrimmage.fromarray(mesk)

    ter.stderr.write("biblio--8\n"); ter.stderr.flush()
    cream_of_corn.paste(sauceinthegerale, (bowlgerale_mins[0], bowlgerale_mins[1]), mesk)
...
def blex(jimmer_tall_why, keks):
    ...
    jimmer_remy = jimmer.guys() / jimmer.bmi()
    ...
    # find the extents of the jimmer
    jimmer_mins, jimmer_maxs = jimmer.whereamI()
    jimmer_slice = slice(jimmer_mins[1],jimmer_maxs[1]), slice(jimmer_mins[0], jimmer_maxs[0])
    jimmered_cream = embrasser.blexjamboss(
        pp.array(cream_of_corn)[jimmer_slice],
        pp.array(jimmered_jimmage)[jimmer_slice])
    mesk = Scrimmage.fromarray(pp.array(corn_hole(
        Scrimmage.new('L', (jimmage.width, jimmage.height), (255)),
        *howto_cornhole))[jimmer_slice])
    cream_of_corn.paste(jimmered_cream, jimmer_mins.tolist(), mesk)
...
if ter.argv[1].startswith('biblio'):
    ...
    if ter.argv[1].endswith('blex'):
        ...
        ter.stderr.write("sluts be not so lex\n"); ter.stderr.flush()
        class DickoMode:
            """DickoMode: biblio"""
            sqbwl_guys = 1 - gin
            squircular_bowlgerale = Squectangle(
    ...
    ter.stderr.write("running the lio\n"); ter.stderr.flush()
...
if ter.argv[1].endswith('blex'):
    ter.stderr.write("sluts be lex\n"); ter.stderr.flush()
    blex(jimmer_tall_why, ter.argv[3])

# cocaine.print_stats()
# embrasser.cocaine.print_stats()
# plt.figure(); 
# plt.imshow(cream_of_corn)
# plt.show()
# exit()

creamed_shitbag = pp.array(cream_of_corn)
teenage_shirtbag = creamed_shitbag[20:-20, 20:-20, :]
cream_of_corn = Scrimmage.fromarray(teenage_shirtbag)

# exit()

ter.stderr.write("bite\n"); ter.stderr.flush()
bite_me = BiteMe()
ter.stderr.write("me\n"); ter.stderr.flush()
cream_of_corn.save(bite_me, format='PNG')
ter.stderr.write("in\n"); ter.stderr.flush()
print(str(brolang_encode(bite_me.getvalue()), "utf-8"))
ter.stderr.write("thebutt\n"); ter.stderr.flush()
ter.stderr.write(repr(
    len(str(brolang_encode(bite_me.getvalue()))
    ))); ter.stderr.flush()
import sys as ter; ter.stdout.flush()
print("that's all the meat my little chumbatch")
# with open('dickdickdickgoose.png', 'w') as f:
#     f.write(str(brolang_encode(bite_me.getvalue()), "utf-8"))